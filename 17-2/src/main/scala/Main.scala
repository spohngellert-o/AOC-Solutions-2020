...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val coordMap = file.getLines().zipWithIndex.toArray.flatMap {
      case (l, y) => l.zipWithIndex.map({case (c, x) => (x, y, 0, 0) -> (if(c == '#') 1 else 0)})
    }.toMap
  ...
  def getNthCycle(coordMap: Map[(Int, Int, Int, Int), Int], target: Int, cur: Int=0): Map[(Int, Int, Int, Int), Int] = {
  ...
  def getNeighbors(coord: (Int, Int, Int, Int)): List[(Int, Int, Int, Int)] = {
    coord match {
      case (x, y, z, w) => (x - 1 to x + 1).toList.flatMap(cx => {
        (y - 1 to y + 1).toList.flatMap(cy => {
          (z - 1 to z + 1).toList.flatMap(cz => {
            (w - 1 to w + 1).map(cw => (cx, cy, cz, cw))
          })
        })
      })
    }
